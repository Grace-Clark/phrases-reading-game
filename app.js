(() => {
  ...
  const DIRECTIONS = {
    ...
    phrase: "Read this phrase. Then press next!",
    choose: "Pick the picture that matches the phrase. Then press next!",
  };
  ...
  // Step 1 — 6 pictures, audio-only. Student must click every picture before
  // the Next button enables.
  function renderListen() {
    ...
    shuffled.forEach((opt) => {
      ...
      card.type = "button";
      card.dataset.word = opt.word;
      // opt.audio is an optional override for the audio filename base — e.g.
      // word "small_wig_dog" plays audio file "wig_dog.m4a".
  ...
  function renderReport() {
    ...
    table.innerHTML = "<thead><tr><th>Phrase</th><th>Picked</th></tr></thead><tbody></tbody>";
    const tbody = table.querySelector("tbody");
    missed.forEach((a) => {
      const tr = document.createElement("tr");
      tr.innerHTML = `<td class="report-word">${prettify(a.target)}</td><td class="report-word report-wrong">${prettify(a.chosen)}</td>`;
    ...
    copyBtn.addEventListener("click", async () => {
      const lines = [
        `Phrase Reading Game — ${state.score}/${state.total} correct`,
        `Date: ${new Date().toLocaleString()}`,
        "",
        "Missed trials:",
        ...missed.map((a) => `  phrase: ${prettify(a.target)} — picked: ${prettify(a.chosen)}`),
      ];
